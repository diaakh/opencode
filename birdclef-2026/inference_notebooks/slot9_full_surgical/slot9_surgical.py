...
if exp019_csv and birdmae_csv:
    ...
    # SURGICAL SLOT9 BLEND: BirdMAE on 13 classes where labeled-OOF delta > 0.02
    # over next-best model (no global drag, only proven BirdMAE wins).
    # 13/234 classes get BirdMAE 50%; remaining 221 stay pure exp019.
    SURGICAL_CLASSES = {'47158son03', '74113', 'strher2', '47158son20', '47158son11', '326272', 'trsowl', '1491113', '47158son04', '47158son10', '47158son01', '47158son19', 'redjun'}
    W_EXP_WIN, W_BM_WIN = 0.50, 0.50
    R_final = R_exp.copy()
    winner_idx = [i for i, c in enumerate(common) if c in SURGICAL_CLASSES]
    n_winners = len(winner_idx)
    print(f"  Surgical: BirdMAE 50% on {n_winners}/{len(common)} classes, pure exp019 on rest")
    print(f"  Surgical classes: {sorted([common[wi] for wi in winner_idx])}")
    for wi in winner_idx:
        R_final[:, wi] = W_EXP_WIN * R_exp[:, wi] + W_BM_WIN * R_bm[:, wi]
    ...
    print(f"  Wrote submission.csv: {out.shape}")
    diff_std = (R_final - R_exp).std(axis=0)
    n_changed = (diff_std > 1e-9).sum()
    print(f"  Diagnostic: {n_changed} class columns modified from pure exp019")
elif exp019_csv:
