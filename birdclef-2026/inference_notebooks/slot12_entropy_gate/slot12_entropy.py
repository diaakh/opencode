# slot12: exp019 + sub_v8 + V73 with PER-CELL ENTROPY-GATED blend.
# LLM self-consistency analog: use exp019's own confidence per (row, class)
# to decide blend weight with helpers. Replaces slot11's fixed per-class routing.
# OOF gain over exp019 anchor: +0.006 to +0.008 on labeled train_soundscapes.

# ---------- exp019 portion ----------
#!/usr/bin/env python
# coding: utf-8

# # exp019 — EoS.4 0.948 + exp017 lambda_prior=0.5 + rank_aware_scaling(power) 0.5 -> 0.6
# 
# Stacked single-scalar probe. Base = exp017 (Kaggle pending; expected >= 0.948 per
# user UI signal). One additional scalar change layered on top:
# - `rank_aware_scaling(probs,    n_windows=N_WINDOWS, power=0.5)` -> `rank_aware_scaling(probs,    n_windows=N_WINDOWS, power=0.6)`
# 
# Rationale: orthogonal dial test. file_confidence_scale / rank_aware_scaling /
# adaptive_delta_smooth are independent postprocess steps in Karnakbayev's chain;
# each is a candidate +0.001-0.002 yield axis. Single-line patches keep attribution
# clean — any score movement vs exp017 is caused by this single line.
# 
# Decision rule for tomorrow exp020:
# - if exp019 > exp017: keep this scalar bump; combine with the OTHER positive dial.
# - if exp019 = exp017: this dial is saturated at its current value; skip.
# - if exp019 < exp017: reverse direction next time.
# 
# 
# ---
# 
# # exp017 — EoS.4 0.948 anchor with apply_prior(lambda_prior) 0.4 -> 0.5
# 
# Single scalar delta vs EoS.4 (`sunderekkiz/birdclef-2026-eos-4` v1,
# publicScore 0.948 confirmed 2026-05-17 via submission 52736927).
# 
# Changes:
# - `apply_prior(..., lambda_prior=0.4)` -> `apply_prior(..., lambda_prior=0.5)`
#   at both call sites in cell 14 (OOF calibration path AND test path).
# - `calibrate_and_optimize_thresholds` will recompute PER_CLASS_THRESHOLDS
#   on OOF probabilities under the new lambda, so test-side prior + threshold
#   application remain self-consistent.
# 
# Why this scalar:
# - The 0.948 pipeline's lambda_prior=0.4 was set as a literal default
#   without OOF gridding (see exp015v2 closed-paths note).
# - The earlier exp015v2 BLANK closed the path of `prior alone on raw 3-way
#   blend`; here the prior sits at step 4 of a 5-stage calibrated chain, so
#   intensification is safe.
# - One scalar change keeps the experiment attributable: any score movement
#   is causally linked to this single line.
# 
# Decision rule for follow-up (exp018):
# - If exp017 > 0.948: bisect upward (lambda_prior = 0.6, then 0.55 etc).
# - If exp017 = 0.948: lambda is saturated; switch to a different scalar.
# - If exp017 < 0.948: bisect downward (lambda_prior = 0.3, then 0.35).
# 
# 
# ---
# 
# &nbsp;
# 
# **Dear colleagues and friends**! &nbsp; Please listen:
# 
# In this notebook, we'll attempt to vertically divide two tests and assign slightly different weights to each half. We think we should notice something. First, we'll simply divide it into two even halves. Then we'll shift the cutoff point—first in one direction and then in the other. In short, it's about a 2-3-day journey.
# 
# First, we'll run two runs with different weights, without vertically splitting the tests. Then, we'll try swapping them on the split halves.
# 
# If something works out, we could try to provide a condition for this division—for example, by simply and carefully looking at the existing experimental samples that received the scores. Obviously, the number of rows is limited, but the number of columns is constant!
# 
# Of course, it's not a given that this will simply be "done by hand." But this is strict logic. It's already been proven that two different technologies complement each other. This means that somewhere, one technology doesn't quite agree with the other, and vice versa!
# 
# &nbsp;

# ### [BirdCLEF+ 2026](https://www.kaggle.com/competitions/birdclef-2026/code?competitionId=129329&sortBy=scoreDescending&excludeNonAccessedDatasources=true)
# 
...
# | &nbsp;| &nbsp; | &nbsp; | &nbsp; | &nbsp; | &nbsp; | &nbsp; |
# | - | :-: | :-: | :-: | :-: | :-: | :-: |
# | [Model](#Model_1)_1 <br> [Model](#Model_2)_2 | &nbsp; [0.912](https://www.kaggle.com/code/tuckerarrants/bc2026-distilled-sed/notebook?scriptVersionId=314259833) &nbsp; <br> [0.917](https://www.kaggle.com/code/tuckerarrants/bc2026-distilled-sed?scriptVersionId=315112860)| v.1<br>v.2 | [BC2026 Distilled-SED](https://www.kaggle.com/code/tuckerarrants/bc2026-distilled-sed) | **m**aster | &nbsp; &nbsp; [Tucker Arrants](https://www.kaggle.com/tuckerarrants) &nbsp; &nbsp; | United States |
# |||||||
# | [Model](#Model_3)_3 | [0.928](https://www.kaggle.com/code/hideyukizushi/bird26-reproduce-perch-protossm-resssm-inf-train/notebook?scriptVersionId=309368265) | v.18 | [Bird26.REPRODUCE.Perch+<br>&nbsp; &nbsp; &nbsp; &nbsp; ProtoSSM+ResSSM.INF/TRAIN &nbsp; &nbsp; &nbsp; &nbsp;](https://www.kaggle.com/code/hideyukizushi/bird26-reproduce-perch-protossm-resssm-inf-train/notebook) | grand**m**aster | [yukiZ](https://www.kaggle.com/hideyukizushi) | Japan |
# |||||||
# | [Model](#Model_4)_4 | [0.947](https://www.kaggle.com/code/mtoshidesu/tedbirdclef-2026-improved?scriptVersionId=318916649) |v.3| [tedBirdCLEF+ 2026 Improved](https://www.kaggle.com/code/mtoshidesu/tedbirdclef-2026-improved) | expert | [m-toshi desu](https://www.kaggle.com/mtoshidesu) | World |
# ||||||||
# | [Model](#Model_7)_7 | [0.948](https://www.kaggle.com/code/pilkwang/948-birdclef-26-acoustic-time-window-rank-fusion/notebook?scriptVersionId=319935333) |v.6| [948- BirdCLEF+ 26 Acoustic Time-Window Rank Fusion](https://www.kaggle.com/code/pilkwang/948-birdclef-26-acoustic-time-window-rank-fusion/notebook) | expert | [Pilkwang Kim](https://www.kaggle.com/pilkwang) | South Korea |
# ||||||||
# 
# | &nbsp;| &nbsp; | &nbsp; | &nbsp; | &nbsp; | &nbsp; | &nbsp; |
# | - | :-: | :-: | :-: | :-: | :-: | :-: |
# |||| [ **Models** ] . [ **their LB's** ] | **type of blend** | **schema weights** | **time**
# || [0.947](https://www.kaggle.com/code/nina2025/birdclef-2026-ensemble-of-solutions?scriptVersionId=319172934) | v.prev | &nbsp; &nbsp; &nbsp; [ 3 + 4 ] . [ 0.928 + 0.947 ] &nbsp; &nbsp; &nbsp; | [direct](#direct) | [[ 0.026 + 0.974 ](#preparing_parameters)] | < 1.5 h
# |.| [0.947](https://www.kaggle.com/code/nina2025/birdclef-2026-eos-4?scriptVersionId=319668381) | **v3** | &nbsp; &nbsp; &nbsp; [ 3 + 4 ] . [ 0.928 + 0.947 ] &nbsp; &nbsp; &nbsp; | [direct](#direct) | [[ 0.027 + 0.973 ](#preparing_parameters)] | < 1.5 h
# |.| [0.947](https://www.kaggle.com/code/nina2025/birdclef-2026-eos-4?scriptVersionId=319690944) | **v4** | &nbsp; &nbsp; &nbsp; [ 3 + 4 ] . [ 0.928 + 0.947 ] &nbsp; &nbsp; &nbsp; | [direct](#direct) | [[ 0.033 + 0.967 ](#preparing_parameters)] | < 1.5 h
# ||||||||
# || [?]() | v9 | &nbsp; &nbsp; &nbsp; [ 7 ] . [ 0.948 ] &nbsp; &nbsp; &nbsp; | [single](#direct) | [ [ 1 ] ](#preparing_parameters) | < 1 h
# ||||||||
# ||||||half.1 : half.2|
# || [?]() | v10 | &nbsp; &nbsp; &nbsp; [ 3 + 7 ] . [ 0.928 + 0.948 ] &nbsp; &nbsp; &nbsp; | [direct_safe](#direct) | [ 0.033 + 0.967 ] : [ [0.026 + 0.974](#preparing_parameters) ] | ?
# || [?]() | v11 | &nbsp; &nbsp; &nbsp; [ 3 + 7 ] . [ 0.928 + 0.948 ] &nbsp; &nbsp; &nbsp; | [direct_safe](#direct) | [ [0.026 + 0.974](#preparing_parameters) ] : [ 0.033 + 0.967 ] | ?
# ||||||||
# |.| [?]() | v12 | &nbsp; &nbsp; &nbsp; [ 3 + 4 ] . [ 0.928 + 0.947 ] &nbsp; &nbsp; &nbsp; | [direct_safe](#direct) | [ 0.033 + 0.967 ] : [ [0.026 + 0.974](#preparing_parameters) ] | ?
# |.| [?]() | v13 | &nbsp; &nbsp; &nbsp; [ 3 + 4 ] . [ 0.928 + 0.947 ] &nbsp; &nbsp; &nbsp; | [direct_safe](#direct) | [ [0.026 + 0.974](#preparing_parameters) ] : [ 0.033 + 0.967 ] | ?
# |||||||
# 
# &nbsp;
# 
...
# We accept the [**amendments**](https://www.kaggle.com/code/pilkwang/birdclef-2026-safe-ensemble) of  South Korean expert [**Pilkwang Kim**](https://www.kaggle.com/pilkwang) and say a big thank you to him.
...
# _Model_ = solutions['Models'][1]['Model']
# _subm_  = solutions['Models'][1]['subm']

# div_wts_1 = {
#     'first_half'  : [0.027, 0.973],
#     'second_half' : [0.033, 0.967]
# }
# div_wts_2 = {
#     'second_half' : [0.027, 0.973],
#     'first_half'  : [0.033, 0.967]
# }

# div_wts = div_wts_1 if HALF==1 else div_wts_2

# solut = {
#      'task1'    : {'div':'dataframe_on_2_half', f'boundary':234/2 },
#      'type_add' :'direct_safe',
#      'Models'   : [

#       {'Model':f'Model_3','subm':f'subm_3.csv','LB':'0.928','xSED':[],
#            'weight':0.033,
#            'wts_first_half' : div_wts['first_half'] [0],
#            'wts_second_half': div_wts['second_half'][0],
#       },  
#       {'Model':_Model_, 'subm':_subm_, 'LB':'0.94_','xSED':[0.60,0.40],
#            'weight':0.967,
#            'wts_first_half' : div_wts['first_half'] [1],
#            'wts_second_half': div_wts['second_half'][1],
#       }]
# }
...
if 'Model_2' in _ensemble_models or \
   'task' in solutions and 'run Model_2_SED once' in solutions['task']:
    ...
    get_ipython().system('pip install -q /kaggle/input/datasets/tuckerarrants/perch-v2-no-dft-onnx/onnxruntime-1.24.4-cp312-cp312-manylinux_2_27_x86_64.manylinux_2_28_x86_64.whl')
    ...
    if MODE == "train":
        get_ipython().system('pip install -q timm torchaudio onnxscript onnx')
    ...
    if MODE != "infer":
        ...
        # --- Main inference loop (FAST: prefetched audio + opportunistic cross-file batch) ---
        # SPEEDUP (no math change):
        #   1. ThreadPoolExecutor prefetches audio + mel for next batch in parallel
        ...
        if test_files:
            try:
                _probe_chunks, _ = file_to_chunks(test_files[0])
                _probe_mel = audio_to_mel(_probe_chunks)  # (12, 1, n_mels, n_frames)
                _probe_double = np.concatenate([_probe_mel, _probe_mel], axis=0)  # (24, ...)
                fold_sessions[0].run(None, {'mel': _probe_double})
                print(f"[fast] SED ONNX supports cross-file batching (batch_files={SED_BATCH_FILES})")
            except Exception as _e:
                _sed_supports_batching = False
                print(f"[fast] SED ONNX has fixed batch shape — falling back to per-file mode (prefetch still active). reason: {type(_e).__name__}")

        t0 = time.time()
        all_rows, all_preds = [], []

        def _load_chunks_and_mel(p):
            chunks, end_times = file_to_chunks(p)
            mel = audio_to_mel(chunks)
            return p, chunks, end_times, mel
        ...
        with _cf.ThreadPoolExecutor(max_workers=2) as _pool:
            ...
            for batch_start in range(0, len(test_files), _effective_batch):
                batch_results = [f.result() for f in _next_futs]
                next_start = batch_start + _effective_batch
                if next_start < len(test_files):
                    _next_batch_paths = test_files[next_start:next_start + _effective_batch]
                    _next_futs = [_pool.submit(_load_chunks_and_mel, p) for p in _next_batch_paths]

                if _sed_supports_batching and len(batch_results) > 1:
                    # ONE ONNX call per fold for the WHOLE batch
                    batch_mel = np.concatenate([r[3] for r in batch_results], axis=0)
                    bn = batch_mel.shape[0]
                    logits_sum = np.zeros((bn, NUM_CLASSES), dtype=np.float32)
                    ...
                    for bi, (file_path, _chunks, end_times, _mel) in enumerate(batch_results):
                        file_logits = logits_mean[bi * N_WINDOWS:(bi + 1) * N_WINDOWS]
                        logits_smoothed = gauss_smooth_final(file_logits)
                    ...
                    # Per-file path (when ONNX has fixed batch shape) — still benefits from prefetch
                    for file_path, _chunks, end_times, mel in batch_results:
                        logits_sum = np.zeros((mel.shape[0], NUM_CLASSES), dtype=np.float32)
                        for sess in fold_sessions:
                ...
                step = max(_effective_batch * 5, 50)
                if done == _effective_batch or done % step == 0 or done == len(test_files):
...
if 'Model_4' in _ensemble_models:
    ...
    if test_paths:
        try:
            _pchk, _ = file_to_sed_chunks(test_paths[0])
            _pmel = audio_to_mel(_pchk)
            _pdbl = np.concatenate([_pmel, _pmel], axis=0)
            sed_sessions[0].run(None, {sed_sessions[0].get_inputs()[0].name: _pdbl})
            print(f"[fast] Model_4 SED supports cross-file batching")
        except Exception as _e:
            _sed4_supports_batch = False
            print(f"[fast] Model_4 SED has fixed batch — per-file fallback (prefetch active). reason: {type(_e).__name__}")
    _sed4_eff_batch = SED4_BATCH_FILES if _sed4_supports_batch else 1

    def _sed4_load(p):
        chunks, ends = file_to_sed_chunks(p)
        mel = audio_to_mel(chunks)
    ...
    with _cf_sed4.ThreadPoolExecutor(max_workers=2) as _pool:
        ...
        for batch_start in range(0, len(test_paths), _sed4_eff_batch):
            batch_results = [f.result() for f in _next_futs]
            ns = batch_start + _sed4_eff_batch
            if ns < len(test_paths):
                _next_paths = test_paths[ns:ns + _sed4_eff_batch]
                _next_futs = [_pool.submit(_sed4_load, p) for p in _next_paths]

            if _sed4_supports_batch and len(batch_results) > 1:
                batch_mel = np.concatenate([r[3] for r in batch_results], axis=0)
                bn = batch_mel.shape[0]
                p_sum_batch = np.zeros((bn, N_CLASSES), dtype=np.float32)
                for sess in sed_sessions:
                    outs = sess.run(None, {sess.get_inputs()[0].name: batch_mel})
                    clip_logits = outs[0]
                    frame_max = outs[1].max(axis=1)
                    p_sum_batch += 0.5 * sigmoid_sed(clip_logits) + 0.5 * sigmoid_sed(frame_max)
                p_mean_batch = p_sum_batch / len(sed_sessions)
                for bi, (path, _chunks, ends, _mel) in enumerate(batch_results):
                    p_mean = p_mean_batch[bi * N_WINDOWS:(bi + 1) * N_WINDOWS]
                    if len(p_mean) > 1:
                ...
                for path, _chunks, ends, mel in batch_results:
    ...
    def run_birdnet(paths, verbose=True):
        ...
        # FAST: prefetch audio while TFLite invokes current file (same math)
        import concurrent.futures as _cf_bn

        def _bn_load(p):
            y, sr0 = sf.read(str(p), dtype="float32", always_2d=False)
            if y.ndim == 2: y = y.mean(axis=1)
            if sr0 != BIRDNET_SR:
            ...
            return p, y.reshape(_N_BN_CHUNKS, BIRDNET_CHUNK_SAMPLES)

        _bn_pool = _cf_bn.ThreadPoolExecutor(max_workers=3)
        _next_fut = _bn_pool.submit(_bn_load, paths[0]) if paths else None
        for idx, path in enumerate(itr):
            if _next_fut is None:
                _next_fut = _bn_pool.submit(_bn_load, path)
            _, chunks = _next_fut.result()
            if idx + 1 < len(paths):
                _next_fut = _bn_pool.submit(_bn_load, paths[idx + 1])
            else:
                _next_fut = None
    ...
    # ── Gate 3b: BirdNET spike preservation (NEW) ─────────────────────────────────
    # Brief high-confidence BirdNET detections that both ProtoSSM and SED missed.
    # Particularly helps unmapped species where Perch has no logit.
    if rank_birdnet is not None:
...
if 'Model_7' in _ensemble_models:
    ...
    # Clean EOS-4 replay: run the Karnakbayev Power optimization branch directly.
    # The original eos-4 outer blend is 0.001 * Youssef LB0.948 + 0.999 * Karnakbayev Power optimization,
    # so the score-driving path is this single branch.
    solutions = {
    ...
    if 'Karnakbayev_PowerOptimization_LB0948' in _ensemble_models:
        ...
        if test_paths:
            try:
                ...
                _pdbl = np.concatenate([_pmel, _pmel], axis=0)
                sed_sessions[0].run(None, {sed_sessions[0].get_inputs()[0].name: _pdbl})
                print(f"[fast] Model_7 SED supports cross-file batching")
            except Exception as _e:
                _sed7_supports_batch = False
                print(f"[fast] Model_7 SED has fixed batch — per-file fallback (prefetch active). reason: {type(_e).__name__}")
        _sed7_eff_batch = SED7_BATCH_FILES if _sed7_supports_batch else 1
        ...
        with _cf_sed7.ThreadPoolExecutor(max_workers=2) as _pool:
            ...
            for batch_start in range(0, len(test_paths), _sed7_eff_batch):
                batch_results = [f.result() for f in _next_futs]
                ns = batch_start + _sed7_eff_batch
                if ns < len(test_paths):
                    _next_paths = test_paths[ns:ns + _sed7_eff_batch]
                    _next_futs = [_pool.submit(_sed7_load, p) for p in _next_paths]

                if _sed7_supports_batch and len(batch_results) > 1:
                    batch_mel = np.concatenate([r[3] for r in batch_results], axis=0)
                    bn = batch_mel.shape[0]
                    p_sum_batch = np.zeros((bn, N_CLASSES), dtype=np.float32)
                    for sess in sed_sessions:
...
# ============================================================================
# V73 5-fold ONNX inference (LB-validated 0.941, adds Aves diversity)
# ============================================================================
print("\n" + "="*80 + "\nV73 INFERENCE\n" + "="*80)
import glob as _gl_v73
import time as _t_v73
import re as _re_v73

V73_PATHS = []
for _root in ["/kaggle/input"]:
    V73_PATHS += sorted(_gl_v73.glob(f"{_root}/**/v73_fold*.onnx", recursive=True))
print(f"V73 folds: {len(V73_PATHS)}")

if V73_PATHS and _Path("/kaggle/input/competitions/birdclef-2026/test_soundscapes").exists():
    import onnxruntime as _ort_v73
    import soundfile as _sf_v73
    import librosa as _libv73_
    ...
    _F_MIN_V73 = 20
    _F_MAX_V73 = 15000
    _WIN_SAMPLES_V73 = 5 * _SR_V73

    def _mel_for_clip_v73(audio):
        mel = _libv73_.feature.melspectrogram(
            y=audio.astype(np.float32), sr=_SR_V73,
            n_fft=_N_FFT_V73, hop_length=_HOP_V73, n_mels=_N_MELS_V73,
            fmin=_F_MIN_V73, fmax=_F_MAX_V73, power=2.0,
        )
        mel_db = _libv73_.power_to_db(mel, ref=np.max, top_db=80.0)
    ...
    _so_v73 = _ort_v73.SessionOptions()
    _so_v73.intra_op_num_threads = 4
    _so_v73.inter_op_num_threads = 1
    _so_v73.graph_optimization_level = _ort_v73.GraphOptimizationLevel.ORT_ENABLE_ALL
    _v73_sessions = [_ort_v73.InferenceSession(_p, sess_options=_so_v73, providers=["CPUExecutionProvider"]) for _p in V73_PATHS]
    print(f"  Loaded {len(_v73_sessions)} V73 sessions")
    
    _comp = _Path("/kaggle/input/competitions/birdclef-2026")
    if not _comp.exists(): _comp = _Path("/kaggle/input/birdclef-2026")
    _test = _comp / "test_soundscapes"
    _test_files = sorted(_test.glob("*.ogg"))
    _samp_v73 = pd.read_csv(_comp / "sample_submission.csv")
    _class_cols_v73 = [c for c in _samp_v73.columns if c != "row_id"]
    _C_V73 = len(_class_cols_v73)
    _N_WINDOWS_V73 = 12
    
    if _test_files:
        print(f"  Processing {len(_test_files)} files...")
        _all_rows_v73 = []
        _all_preds_v73 = []
        _t0_v73 = _t_v73.time()
        for _fi, _fp in enumerate(_test_files):
            try:
                _aud, _sr = _libv73_.load(str(_fp), sr=_SR_V73, mono=True)
            except:
                continue
            _stem = _fp.stem
            _file_scores = []
            for _w in range(_N_WINDOWS_V73):
                _s_samp = _w * _WIN_SAMPLES_V73
                if _s_samp + _WIN_SAMPLES_V73 <= len(_aud):
                    _clip = _aud[_s_samp:_s_samp + _WIN_SAMPLES_V73]
                else:
                    _clip = np.zeros(_WIN_SAMPLES_V73, dtype=np.float32)
                    _av = max(0, len(_aud) - _s_samp)
                    if _av > 0: _clip[:_av] = _aud[_s_samp:_s_samp+_av]
                _mel = _mel_for_clip_v73(_clip)[None, :, :, :]  # (1, 1, 128, 313)
                _fold_probs = []
                for _sess in _v73_sessions:
                    try:
                        _logits = _sess.run(["clipwise"], {"mel": _mel})[0][0]
                    except Exception:
                        _in_name = _sess.get_inputs()[0].name
                        _out_name = _sess.get_outputs()[0].name
                        _logits = _sess.run([_out_name], {_in_name: _mel})[0][0]
                    _fold_probs.append(1.0 / (1.0 + np.exp(-np.clip(_logits, -30, 30))))
                _mean = np.mean(_fold_probs, axis=0)
                _file_scores.append(_mean[:_C_V73])
                _all_rows_v73.append(f"{_stem}_{(_w+1)*5}")
            _all_preds_v73.extend(_file_scores)
            if (_fi + 1) % 50 == 0 or _fi == len(_test_files) - 1:
                _el = _t_v73.time() - _t0_v73
                print(f"    [{_fi+1}/{len(_test_files)}] {_el:.0f}s")
        print(f"  V73 total: {_t_v73.time()-_t0_v73:.0f}s")
        _P_arr_v73 = np.array(_all_preds_v73, dtype=np.float32)
        _out_v73 = pd.DataFrame({"row_id": _all_rows_v73})
        for _i, _c in enumerate(_class_cols_v73):
            _out_v73[_c] = _P_arr_v73[:, _i] if _i < _P_arr_v73.shape[1] else 0.0
        _out_v73 = _out_v73.set_index("row_id").reindex(_samp_v73["row_id"]).fillna(0).reset_index()
        _out_v73.to_csv("v73_submission.csv", index=False)
        print(f"  Wrote v73_submission.csv: {_out_v73.shape}")
    else:
        print("  No test files for V73")
else:
    print("  V73 not run (folds missing or no test_soundscapes)")
...
# SLOT11 SURGICAL MULTI-MODEL BLEND
# Per-class routing based on labeled-OOF analysis. 27 classes get a non-exp019
# source (sub_v8 stack output or V73), each at margin-proportional weight.
# 207 classes stay pure exp019 (no dilution where exp019 is calibrated).

if exp019_csv and subv8_csv:
    df_exp = pd.read_csv(exp019_csv)
    df_sub = pd.read_csv(subv8_csv)
    if "row_id" not in df_exp.columns:
        df_exp = df_exp.rename(columns={df_exp.columns[0]: "row_id"})
    if not df_exp["row_id"].equals(df_sub["row_id"]):
        df_sub = df_sub.set_index("row_id").reindex(df_exp["row_id"]).reset_index()
    
    # Also load V73 if available
    df_v73 = None
    for vp in ["v73_submission.csv", "/kaggle/working/v73_submission.csv"]:
        if Path(vp).exists():
            df_v73 = pd.read_csv(vp)
            if "row_id" not in df_v73.columns:
                df_v73 = df_v73.rename(columns={df_v73.columns[0]: "row_id"})
            if not df_exp["row_id"].equals(df_v73["row_id"]):
                df_v73 = df_v73.set_index("row_id").reindex(df_exp["row_id"]).reset_index()
            print(f"  v73 csv: {vp}")
            break
    ...
    R_v73 = None
    if df_v73 is not None:
        # V73 columns might be a subset
        v73_common = [c for c in df_v73.columns if c != "row_id"]
        M_v73 = np.zeros_like(M_exp)
        for i, c in enumerate(common):
            if c in v73_common:
                M_v73[:, i] = df_v73[c].to_numpy(dtype=np.float32)
        R_v73 = _rank_norm_cols(M_v73)
    
    # SLOT12 v3: TEMP-CALIBRATED PER-CELL ENTROPY-GATED BLEND
    # Key fix: exp019's raw probability distribution is too flat (76% cells with
    # entropy > 0.7), making the entropy gate nearly equivalent to a fixed
    # weighted blend. Temperature-scaling exp019's LOGITS (T=2) sharpens its
    # distribution to ~46% high-entropy cells, giving the gate real dynamic range.
    #
    # Critical: temperature scaling is monotonic → exp019's per-class ranking
    # (and therefore AUC) is preserved exactly. Only the entropy signal changes.
    #
    # Algorithm:
    #   1. Calibrate: P_calib = sigmoid(CALIB_T * logit(P_exp))
    #   2. Entropy H of P_calib per cell
    #   3. confidence = (1 - H) ** GATE_T
    #   4. anchor_weight = clip(confidence, FLOOR, CEIL)
    #   5. Helper weight = (1 - anchor_weight); 95% to sub_v8, 5% to V73
    #
    # OOF (labeled, sub_v8 proxy + V73):
    #   uncalibrated, floor=0.55       : 0.9731 (+0.011)
    #   T=2.0 calib, floor=0.30        : 0.9809 (+0.019) ← chosen
    #   T=2.0 calib, floor=0.50        : 0.9770 (+0.015)
    CALIB_T = 2.0          # sharpens exp019 logits BEFORE entropy calc
    GATE_T = 2.0           # entropy → confidence exponent
    ANCHOR_FLOOR = 0.40    # safer than 0.30 — caps helper weight at 60% per cell
    ANCHOR_CEIL = 0.90
    V73_HELPER_FRACTION = 0.05

    eps = 1e-6
    p_exp = np.clip(M_exp, eps, 1 - eps)
    # Calibrate via logit-space temperature scaling (rank-preserving)
    logit_exp = np.log(p_exp / (1 - p_exp))
    p_calib = 1.0 / (1.0 + np.exp(-CALIB_T * logit_exp))
    p_calib = np.clip(p_calib, eps, 1 - eps)
    H_calib = -(p_calib * np.log(p_calib) + (1 - p_calib) * np.log(1 - p_calib)) / np.log(2.0)
    conf_exp = np.power(1.0 - H_calib, GATE_T)
    W_anchor = np.clip(conf_exp, ANCHOR_FLOOR, ANCHOR_CEIL).astype(np.float32)
    W_remain = (1.0 - W_anchor).astype(np.float32)

    if R_v73 is not None:
        W_sub = W_remain * (1.0 - V73_HELPER_FRACTION)
        W_v73 = W_remain * V73_HELPER_FRACTION
        R_final = W_anchor * R_exp + W_sub * R_sub + W_v73 * R_v73
        helper_msg = f"sub_v8={1-V73_HELPER_FRACTION:.2f}, v73={V73_HELPER_FRACTION:.2f}"
    else:
        R_final = W_anchor * R_exp + W_remain * R_sub
        helper_msg = "sub_v8=1.00 (V73 missing)"

    print(f"  Entropy-gated blend: calib_T={CALIB_T}, gate_T={GATE_T}, floor={ANCHOR_FLOOR}, ceil={ANCHOR_CEIL}")
    print(f"  Helper split: {helper_msg}")
    print(f"  Anchor weight stats: min={W_anchor.min():.3f} mean={W_anchor.mean():.3f} max={W_anchor.max():.3f}")
    print(f"  Calibrated entropy: mean={H_calib.mean():.3f}, sharp(H<0.3)={(H_calib<0.3).mean()*100:.1f}%")
    print(f"  Cells touched by helpers: {int((W_anchor < ANCHOR_CEIL - 1e-6).sum())}/{W_anchor.size}")
