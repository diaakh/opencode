# slot7: exp019 + BirdMAE + Perch 2.0 3-way blend (60/25/15)
# Target LB 0.96+, OOF 0.9827
...
# ============================================================================
# Bridge: snapshot exp019's output before BirdMAE/Perch run
# ============================================================================
import shutil as _shutil
from pathlib import Path as _Path
for _p in ["submission.csv", "/kaggle/working/submission.csv"]:
    if _Path(_p).exists():
        _shutil.copy(_p, "exp019_submission.csv")
        print(f"[bridge] copied {_p} -> exp019_submission.csv")
        break


# ---------- BirdMAE portion (with _BM_NO_TEST flag) ----------
...
if not test_files:
    ...
    out = samp.copy()
    out.iloc[:, 1:] = 0.0
    out.to_csv("birdmae_submission.csv", index=False)
    _BM_NO_TEST = True
else:
    _BM_NO_TEST = False
...
if not _BM_NO_TEST:
    ...
    all_preds = []
    t0 = time.time()
    BATCH_FILES = 2  # 2 files × 12 windows = 24 windows (measured optimal: 290s vs 297@4)
...
# ---------- Perch 2.0 ONNX portion ----------

# ============================================================================
# Perch 2.0 (multi-taxa, Aug 2025) inference via ONNX
# ============================================================================
print("\n" + "="*80 + "\nPERCH 2.0 ONNX INFERENCE\n" + "="*80)
import os as _os, sys as _sys, glob as _gl, time as _t, subprocess as _sp
import numpy as _np
import pandas as _pd
import soundfile as _sf

# Install onnxruntime from Nina's bundled wheel (if not already installed)
_ort_wheel = None
for _w in _gl.glob("/kaggle/input/**/onnxruntime-1.24*.whl", recursive=True):
    _ort_wheel = _w; break
try:
    import onnxruntime as _ort
    print(f"onnxruntime {_ort.__version__} already importable")
except ImportError:
    if _ort_wheel:
        _sp.check_call([_sys.executable, "-m", "pip", "install", "-q", "--no-deps", _ort_wheel])
        import onnxruntime as _ort
        print(f"installed onnxruntime {_ort.__version__}")

# Find Perch 2.0 ONNX (413MB version)
_P20 = None
for _p in _gl.glob("/kaggle/input/**/perch_v2_no_dft.onnx", recursive=True):
    _sz = _os.path.getsize(_p) / 1e6
    if 350 < _sz < 450:
        _P20 = _p; print(f"Perch 2.0 ONNX: {_p} ({_sz:.0f}MB)"); break
assert _P20, "Perch 2.0 ONNX not attached (nina2025/perch-v2-no-dft-v3-onnx)"

_so = _ort.SessionOptions()
_so.intra_op_num_threads = 4
_so.inter_op_num_threads = 1
_so.graph_optimization_level = _ort.GraphOptimizationLevel.ORT_ENABLE_ALL
_p20_sess = _ort.InferenceSession(_P20, sess_options=_so, providers=["CPUExecutionProvider"])
_in = _p20_sess.get_inputs()[0].name
_out_names = [o.name for o in _p20_sess.get_outputs()]
_LBL_IDX = _out_names.index("label") if "label" in _out_names else 3
print(f"Perch 2.0 input: {_in}, label idx: {_LBL_IDX}")

# Find Perch labels (from google/bird-vocalization-classifier perch_v2_cpu model assets)
_PERCH_SCI = None; _PERCH_EBIRD = None
for _p in _gl.glob("/kaggle/input/**/labels.csv", recursive=True):
    try:
        _df = _pd.read_csv(_p)
        if 14000 < len(_df) < 15000:
            _PERCH_SCI = _df; print(f"Perch sci labels: {_p}"); break
    except: pass
for _p in _gl.glob("/kaggle/input/**/perch_v2_ebird_classes.csv", recursive=True):
    try:
        _df = _pd.read_csv(_p)
        if 14000 < len(_df) < 15000:
            _PERCH_EBIRD = _df; print(f"Perch ebird labels: {_p}"); break
    except: pass

# Reuse exp019's already-loaded sample submission / class list
_samp = _pd.read_csv(SAMPLE) if "SAMPLE" in dir() else _pd.read_csv(SAMPLE_PATH if "SAMPLE_PATH" in dir() else _Path("/kaggle/input/competitions/birdclef-2026/sample_submission.csv"))
_class_cols = [_c for _c in _samp.columns if _c != "row_id"]
_tax = _pd.read_csv(_Path("/kaggle/input/competitions/birdclef-2026") / "taxonomy.csv")
_C = len(_class_cols)
_bc_idx = {_c: _i for _i, _c in enumerate(_class_cols)}

_sci_to_pi = {}; _ebird_to_pi = {}
if _PERCH_SCI is not None:
    for _pi, _s in enumerate(_PERCH_SCI.iloc[:, 0]):
        _sci_to_pi[str(_s).strip().lower()] = _pi
if _PERCH_EBIRD is not None:
    for _pi, _s in enumerate(_PERCH_EBIRD.iloc[:, 0]):
        _ebird_to_pi[str(_s).strip().lower()] = _pi
_bc_to_perch = {}
for _bi, _bc in enumerate(_class_cols):
    _sci_row = _tax[_tax["primary_label"].astype(str) == str(_bc)]
    _sci = str(_sci_row.iloc[0]["scientific_name"]).strip().lower() if len(_sci_row) > 0 else ""
    if _sci and _sci in _sci_to_pi: _bc_to_perch[_bi] = _sci_to_pi[_sci]; continue
    if str(_bc).lower() in _ebird_to_pi: _bc_to_perch[_bi] = _ebird_to_pi[str(_bc).lower()]; continue
print(f"BC2026 -> Perch 2.0 mapping: {len(_bc_to_perch)}/{_C}")

# Run on test_soundscapes
...
_SR = 32000; _WIN = 5 * _SR; _NWIN = 12

if _test_files:
    print(f"\nProcessing {len(_test_files)} test files with Perch 2.0...")
    import re as _re
    ...
    while _i < len(_test_files):
        _batch_files = _test_files[_i:_i+_BATCH]
        _all_wins = []
        _row_ids_in_batch = []
        for _fp in _batch_files:
            try:
                _aud, _sr = _sf.read(str(_fp))
                if _aud.ndim > 1: _aud = _aud.mean(axis=1)
                _aud = _aud.astype(_np.float32)
            except:
                _aud = _np.zeros(_NWIN * _WIN, dtype=_np.float32)
            _stem = _fp.stem
            for _w in range(_NWIN):
                _s = _w * _WIN; _e = _s + _WIN
                if _e <= len(_aud):
                    _clip = _aud[_s:_e]
                else:
                    _clip = _np.zeros(_WIN, dtype=_np.float32)
                    _av = max(0, len(_aud) - _s)
                    if _av > 0: _clip[:_av] = _aud[_s:_s+_av]
                _all_wins.append(_clip)
                _row_ids_in_batch.append(f"{_stem}_{(_w+1)*5}")
        _batch_arr = _np.stack(_all_wins).astype(_np.float32)
        _out = _p20_sess.run(None, {_in: _batch_arr})
        _logits = _out[_LBL_IDX]
        _probs = 1.0 / (1.0 + _np.exp(-_logits))
        _bc_probs = _np.zeros((_logits.shape[0], _C), dtype=_np.float32)
        for _bi, _pi in _bc_to_perch.items():
            _bc_probs[:, _bi] = _probs[:, _pi]
        _all_rows.extend(_row_ids_in_batch)
        _all_preds.extend(_bc_probs)
        _i += _BATCH
        if (_i // _BATCH) % 10 == 0:
            _el = _t.time() - _t0
            _r = _i / _el
            print(f"  [{_i}/{len(_test_files)}] {_el:.0f}s rate={_r:.2f}files/s eta={(len(_test_files)-_i)/max(_r,0.01):.0f}s")
    print(f"\nPerch 2.0 total: {_t.time()-_t0:.0f}s")
    _P_arr = _np.array(_all_preds, dtype=_np.float32)
    _out_df = _pd.DataFrame({"row_id": _all_rows})
    for _bi, _c in enumerate(_class_cols):
        _out_df[_c] = _P_arr[:, _bi]
    # Align to sample
    _out_df = _out_df.set_index("row_id").reindex(_samp["row_id"]).fillna(0).reset_index()
    _out_df.to_csv("perch20_submission.csv", index=False)
    print(f"Wrote perch20_submission.csv: {_out_df.shape}")
else:
    print("No test files - emitting all-zero perch20")
    _out_df = _samp.copy(); _out_df.iloc[:, 1:] = 0.0
    _out_df.to_csv("perch20_submission.csv", index=False)
...
# ============================================================================
# FINAL 3-WAY BLEND: exp019 (0.60) + BirdMAE (0.25) + Perch 2.0 (0.15)
# ============================================================================
print("\n" + "="*80)
print("FINAL 3-WAY BLEND: exp019/BirdMAE/Perch 2.0 = 0.60 / 0.25 / 0.15")
print("="*80)
import pandas as pd
import numpy as np
...
def _load_first_existing(names):
    for n in names:
        if Path(n).exists():
            return pd.read_csv(n), n
    return None, None

df_exp, exp_path = _load_first_existing(["exp019_submission.csv", "/kaggle/working/exp019_submission.csv"])
df_bm, bm_path = _load_first_existing(["birdmae_submission.csv", "/kaggle/working/birdmae_submission.csv"])
df_p20, p20_path = _load_first_existing(["perch20_submission.csv", "/kaggle/working/perch20_submission.csv"])
print(f"  exp019: {exp_path}, shape={df_exp.shape if df_exp is not None else None}")
print(f"  birdmae: {bm_path}, shape={df_bm.shape if df_bm is not None else None}")
print(f"  perch20: {p20_path}, shape={df_p20.shape if df_p20 is not None else None}")

dfs = [d for d in [df_exp, df_bm, df_p20] if d is not None]
weights = []
if df_exp is not None: weights.append(0.60)
if df_bm is not None: weights.append(0.25)
if df_p20 is not None: weights.append(0.15)
total_w = sum(weights)
weights = [w/total_w for w in weights]
print(f"  Effective weights: {weights}")

if not dfs:
    raise RuntimeError("No submission CSVs found")

# Pick reference (first available) for row order
df_ref = df_exp if df_exp is not None else (df_bm if df_bm is not None else df_p20)
if "row_id" not in df_ref.columns:
    df_ref = df_ref.rename(columns={df_ref.columns[0]: "row_id"})
...
for df, w in zip(dfs, weights):
    ...
    if not df["row_id"].equals(df_ref["row_id"]):
        df = df.set_index("row_id").reindex(df_ref["row_id"]).reset_index()
    aligned.append((df, w))

common = [c for c in df_ref.columns if c != "row_id" and all(c in d.columns for d, _ in aligned)]
print(f"  Common class cols: {len(common)}")

R_final = np.zeros((len(df_ref), len(common)), dtype=np.float32)
for df, w in aligned:
    M = df[common].to_numpy(dtype=np.float32)
    R = _rank_norm_cols(M)
    R_final += w * R

out = df_ref[["row_id"]].copy()
for i, c in enumerate(common):
    out[c] = R_final[:, i]
out.to_csv("submission.csv", index=False)
print(f"  Wrote submission.csv: {out.shape}")
print(f"  Range: [{R_final.min():.4f}, {R_final.max():.4f}]")
