# slot6: exp019 + BirdMAE + 70/30 blend
# exp019 LB 0.949 anchor + BirdMAE 0.9721 OOF (SOTA bird-pretrained MAE)
...
# ============================================================================
# FINAL BLEND: exp019 (70%) + BirdMAE (30%) rank-blend
# ============================================================================
print("\n" + "="*80)
print("FINAL BLEND: exp019 + BirdMAE rank-norm 70/30")
print("="*80)
import pandas as pd
import numpy as np
...
birdmae_csv = None
for p in ["birdmae_submission.csv", "/kaggle/working/birdmae_submission.csv"]:
    if Path(p).exists(): birdmae_csv = p; break

print(f"  exp019: {exp019_csv}")
print(f"  birdmae: {birdmae_csv}")

if exp019_csv and birdmae_csv:
    df_exp = pd.read_csv(exp019_csv)
    df_bm = pd.read_csv(birdmae_csv)
    if "row_id" not in df_exp.columns:
        df_exp = df_exp.rename(columns={df_exp.columns[0]: "row_id"})
    if not df_exp["row_id"].equals(df_bm["row_id"]):
        df_bm = df_bm.set_index("row_id").reindex(df_exp["row_id"]).reset_index()
    common = [c for c in df_exp.columns if c != "row_id" and c in df_bm.columns]
    print(f"  Common cols: {len(common)}")
    M_exp = df_exp[common].to_numpy(dtype=np.float32)
    M_bm = df_bm[common].to_numpy(dtype=np.float32)
    R_exp = _rank_norm_cols(M_exp)
    R_bm = _rank_norm_cols(M_bm)
    
    # SLOT10: AUC-margin-weighted per-class blend
    # For each surgical class, BirdMAE gets weight proportional to its labeled-OOF
    # AUC delta over next-best model (formula: clip(0.30 + 2*delta, 0.35, 0.65))
    # Larger margin -> trust BirdMAE more (e.g., strher2 delta=0.178 -> weight 0.65)
    # Smaller margin -> conservative blend (delta=0.02 -> weight 0.35)
    # Default classes: pure exp019 (weight 1.0 / 0.0)
    CLASS_BM_WEIGHTS = {'1491113': 0.35, '326272': 0.359, '47158son01': 0.381, '47158son03': 0.375, '47158son04': 0.4, '47158son10': 0.35, '47158son11': 0.396, '47158son19': 0.35, '47158son20': 0.35, '517063': 0.41, '74113': 0.36, 'redjun': 0.35, 'strher2': 0.65, 'trsowl': 0.384}
    R_final = R_exp.copy()
    n_modified = 0
    for i, c in enumerate(common):
        w_bm = CLASS_BM_WEIGHTS.get(c, 0.0)
        if w_bm > 0.0:
            R_final[:, i] = (1.0 - w_bm) * R_exp[:, i] + w_bm * R_bm[:, i]
            n_modified += 1
    print(f"  Entropy-weighted: BirdMAE blended on {n_modified} classes with per-class margin-proportional weights")
    ...
    print("  Used exp019 alone")
elif birdmae_csv:
    _shutil.copy(birdmae_csv, "submission.csv")
    print("  Used birdmae alone")
