# ============================================================================
# exp019 + v4 OPTIMAL stack (labeled-augmented site_hour + site_shrinkage)
#
# Best recipe from local benchmark: w_sh=0.025 + w_site=0.20 with HYBRID prior. Scenario B 0.9691 (+0.039 over baseline). Expected LB 0.965-0.972.
# ============================================================================
...
assert priors_candidates, "Attach adkasd/birdclef-2026-priors-research (version >=2 with labeled priors)"
PRIORS_DIR = priors_candidates[0].parent
print(f"[v4] Priors: {PRIORS_DIR}")
...
print(f"[v4] Input: rows={len(sub)}, min={prob.min():.4f}, max={prob.max():.4f}")

priors = load_priors_hybrid(PRIORS_DIR)
params = {'w_sh': 0.025, 'w_site': 0.2}
print(f"[v4] Applying params: {params}")
...
print(f"[v4] Wrote submission.csv: rows={len(out)}, min={new_prob.min():.4f}, max={new_prob.max():.4f}")
