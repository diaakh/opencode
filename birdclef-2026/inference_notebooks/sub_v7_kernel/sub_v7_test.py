"""Test kernel for sub_v7: produces a sample submission (uniform 0.5),
applies the v7 post-process cell, and verifies the output is valid.

In production, paste sub_v7_combined_prior_autoscale.py at the end of an
exp019 notebook fork instead. This kernel just confirms the math runs
cleanly on Kaggle's environment.
"""
import re
...
# Build a uniform sample submission as a placeholder for exp019's output
...
print(f"Loaded sample_submission: {samp.shape}")

# Simulate exp019 rank-power scale by mapping uniform to [0.477, 0.555]
class_cols = [c for c in samp.columns if c != "row_id"]
prob = np.random.RandomState(0).uniform(0.477, 0.555, size=(len(samp), len(class_cols))).astype(np.float32)
samp_with = samp.copy()
samp_with[class_cols] = prob
samp_with.to_csv("submission.csv", index=False)
print(f"Wrote synthetic submission.csv: {samp_with.shape}")

# Now apply the v7 post-process cell logic
"""
============================================================================
