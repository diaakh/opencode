...
# === LABELED OOF PATCH ===
def _labeled_oof_files(base):
    """Return paths of the 66 train_soundscapes files that have labels."""
    import pandas as _pd
    labels_csv = base / "train_soundscapes_labels.csv"
    if not labels_csv.exists():
        print("[LABELED-OOF] labels CSV not found, falling back to first 20 train_soundscapes")
        from pathlib import Path as _P
        return sorted((base / "train_soundscapes").glob("*.ogg"))[:20]
    labels = _pd.read_csv(labels_csv).drop_duplicates()
    names = sorted(labels["filename"].unique())
    paths = [base / "train_soundscapes" / n for n in names if (base / "train_soundscapes" / n).exists()]
    print(f"[LABELED-OOF] selected {len(paths)} labeled files (of {len(names)} in CSV)")
    return paths
# === END LABELED OOF PATCH ===
...
if 'Model_3' in _ensemble_models:
    ...
    if len(test_paths) == 0:
        ...
        test_paths = _labeled_oof_files(BASE)
    else:
...
if 'Model_4' in _ensemble_models:
    ...
    # === LABELED OOF SAVE HOOK (model4_pre_align) ===
    try:
        import numpy as _np
        _data_cols = [c for c in sub.columns if c != 'row_id']
        if 'row_id' in sub.columns:
            _row_ids = sub['row_id'].astype(str).tolist()
            _arr = sub[_data_cols].to_numpy(dtype=_np.float32)
        else:
            _row_ids = sub.index.astype(str).tolist()
            _arr = sub[_data_cols].to_numpy(dtype=_np.float32) if _data_cols else sub.to_numpy(dtype=_np.float32)
        _out_path = "/kaggle/working/labeled_oof_model4_pre_align.npz"
        _np.savez_compressed(_out_path, row_ids=_np.array(_row_ids), P=_arr, classes=_np.array(_data_cols))
        print(f"[LABELED-OOF/model4_pre_align] saved shape={_arr.shape} to " + _out_path)
    except Exception as _e:
        print(f"[LABELED-OOF/model4_pre_align] save failed:", _e)
    # === END LABELED OOF SAVE HOOK (model4_pre_align) ===
...
if 'Model_7' in _ensemble_models:
    ...
    if 'Karnakbayev_PowerOptimization_LB0948' in _ensemble_models:
        ...
        # === LABELED OOF SAVE HOOK (model7_pre_align) ===
        try:
            import numpy as _np
            _data_cols = [c for c in sub.columns if c != 'row_id']
            if 'row_id' in sub.columns:
                ...
                _row_ids = sub.index.astype(str).tolist()
                _arr = sub[_data_cols].to_numpy(dtype=_np.float32) if _data_cols else sub.to_numpy(dtype=_np.float32)
            _out_path = "/kaggle/working/labeled_oof_model7_pre_align.npz"
            _np.savez_compressed(_out_path, row_ids=_np.array(_row_ids), P=_arr, classes=_np.array(_data_cols))
            print(f"[LABELED-OOF/model7_pre_align] saved shape={_arr.shape} to " + _out_path)
        except Exception as _e:
            print(f"[LABELED-OOF/model7_pre_align] save failed:", _e)
        # === END LABELED OOF SAVE HOOK (model7_pre_align) ===
    ...
    # === LABELED OOF SAVE HOOK ===
    try:
        import numpy as _np
        _row_col = "row_id"
        if isinstance(submission, pd.DataFrame):
            if _row_col in submission.columns:
                _row_ids = submission[_row_col].astype(str).tolist()
                _data_cols = [c for c in submission.columns if c != _row_col]
                _arr = submission[_data_cols].to_numpy(dtype=_np.float32)
            else:
                _row_ids = submission.index.astype(str).tolist()
                _data_cols = submission.columns.tolist()
                _arr = submission.to_numpy(dtype=_np.float32)
            _out = "/kaggle/working/labeled_oof_exp019_raw.npz"
            _np.savez_compressed(_out, row_ids=_np.array(_row_ids), P=_arr,
                                  classes=_np.array(_data_cols))
            print(f"[LABELED-OOF] saved raw submission: {_arr.shape} -> {_out}")
    except Exception as _e:
        print(f"[LABELED-OOF] raw-save hook failed: {_e}")
    # === END LABELED OOF SAVE HOOK ===
    try:
        final_submission = write_final_submission(submission, "submission.csv")
    except Exception as _e:
        print(f"[LABELED-OOF] write_final_submission failed (likely row_id mismatch w/ sample_submission): {_e}")
...
if not _single_solution:
    ...
    # === LABELED OOF SAVE HOOK 2 ===
    try:
        import numpy as _np
        if isinstance(submission, pd.DataFrame):
            _row_ids = (submission.index.astype(str).tolist() if submission.index.name == 'row_id' 
                        else submission.get('row_id', _np.arange(len(submission))).astype(str).tolist())
            _data_cols = [c for c in submission.columns if c != 'row_id']
            _arr = submission[_data_cols].to_numpy(dtype=_np.float32) if 'row_id' in submission.columns else submission.to_numpy(dtype=_np.float32)
            _out = "/kaggle/working/labeled_oof_exp019_blend.npz"
            _np.savez_compressed(_out, row_ids=_np.array(_row_ids), P=_arr, classes=_np.array(_data_cols))
            print(f"[LABELED-OOF] saved blend submission: {_arr.shape} -> {_out}")
    except Exception as _e:
        print(f"[LABELED-OOF] blend-save hook failed: {_e}")
    try:
        submission.to_csv(f"submission.csv", index = True)
    except Exception as _e:
        print(f'[LABELED-OOF] blend write_final_submission failed: {_e}')
