...
if exp019_csv and birdmae_csv:
    ...
    # SURGICAL BLEND: BirdMAE 50% only on classes where labeled OOF showed clear wins.
    # exp019 calibration preserved on 229/234 classes to avoid drag.
    BIRDMAE_WINNERS = {"326272", "1491113", "22961", "24279", "555146"}
    W_EXP_WIN, W_BM_WIN = 0.50, 0.50
    R_final = R_exp.copy()
    winner_idx = [i for i, c in enumerate(common) if c in BIRDMAE_WINNERS]
    n_winners = len(winner_idx)
    print(f"  Surgical: BirdMAE 50% on {n_winners} winner classes ({sorted(BIRDMAE_WINNERS)}), pure exp019 on {len(common)-n_winners} others")
