# AUTO-GENERATED full-run kernel (env header + main body).
# DIVERSITY MEMBER: tf_efficientnetv2_s.in21k student (vs the b0 sibling) trained on the SAME
# Nikita noisy-student recipe over distilled-SED pseudo-labeled train_soundscapes. effv2s is a
# fused-MBConv / in21k-pretrained backbone => different inductive bias + feature space than b0,
# giving ensemble orthogonality on the SOUNDSCAPE domain (the actual scoring distribution).
import os
os.environ.setdefault("NS_MODEL", "tf_efficientnetv2_s.in21k")
os.environ.setdefault("NS_LIMIT_FILES", "4000")
os.environ.setdefault("NS_ROUNDS", "2")
os.environ.setdefault("NS_EPOCHS", "4")
os.environ.setdefault("NS_BATCH", "48")   # effv2s ~21M params; 48 keeps headroom on a 16GB P100 with 5 GPU SED folds
os.environ.setdefault("NS_WORKERS", "4")
os.environ.setdefault("NS_ADD_FOCAL", "1")
os.environ.setdefault("NS_FOCAL_MAX_FILES", "15")
os.environ.setdefault("NS_SED_GPU", "1")
...
#   STAGE B  train the noisy-student SED student (tf_efficientnetv2_s.in21k, 1ch log-mel, AMP, channels_last)
...
def main():
    ...
    # model-aware artifact name so the effv2s student doesn't collide with the b0 sibling
    safe_model = re.sub(r"[^A-Za-z0-9]+", "_", model_name).strip("_")
    best_path = out_dir / f"ns_student_{safe_model}_fold1_fp16.pt"
