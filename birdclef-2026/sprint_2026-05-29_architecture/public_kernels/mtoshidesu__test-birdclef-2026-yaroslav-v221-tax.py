...
# === MD ===
# # exp_071: eos5_prior065_rankpower065
# 
# Based on `exp_069_eos5_prior065_microblend`.
# 
...
# ---- CELL ----
# Display conceptual architecture diagram from Kaggle dataset

from IPython.display import Image, display, Markdown

IMG_PATH = "/kaggle/input/datasets/mtoshidesu/birdclef-2026-2/ChatGPT Image 2026528 01_11_22.png"

display(Markdown("## Conceptual Intelligence Architecture"))
display(Markdown(
    "The following diagram summarizes the high-level logic of the BirdCLEF 2026 inference pipeline, "
    "including acoustic feature extraction, model fusion, and taxonomy-aware post-processing."
))
...
# | &nbsp;| &nbsp; | &nbsp; | &nbsp; | &nbsp; | &nbsp; | &nbsp; |
# | - | :-: | :-: | :-: | :-: | :-: | :-: |
# | [Model](#Model_1)_1| [0.917](https://www.kaggle.com/code/tuckerarrants/bc2026-distilled-sed?scriptVersionId=315112860)| v.1 | [BC2026 <br> Distilled-SED](https://www.kaggle.com/code/tuckerarrants/bc2026-distilled-sed) | **m**aster | [Tucker Arrants](https://www.kaggle.com/tuckerarrants) | United States |
# |||||||
# | [Model](#Model_2)_2 | [0.928](https://www.kaggle.com/code/hideyukizushi/bird26-reproduce-perch-protossm-resssm-inf-train/notebook?scriptVersionId=309368265) | v.18 | [Bird26.REPRODUCE.Perch+<br>ProtoSSM+ResSSM.INF/TRAIN](https://www.kaggle.com/code/hideyukizushi/bird26-reproduce-perch-protossm-resssm-inf-train/notebook) | grand**m**aster | [yukiZ](https://www.kaggle.com/hideyukizushi) | Japan |
# |||||||
# | [Model](#Model_3)_3 | [0.948](https://www.kaggle.com/code/mtoshidesu/birdclef-2026-visual-cpu-inference?scriptVersionId=319957042) |v.6| [BirdCLEF 2026 . Visual CPU Inference](https://www.kaggle.com/code/mtoshidesu/birdclef-2026-visual-cpu-inference) | expert | [m-toshi desu](https://www.kaggle.com/mtoshidesu) | World |
# |||||||
# | [Model](#Model_4)_4 | [0.948](https://www.kaggle.com/code/pilkwang/birdclef-26-acoustic-time-window-rank-fusion?scriptVersionId=319935011) |v.5| [BirdCLEF+ 26 Acoustic Time-Window Rank Fusion](https://www.kaggle.com/code/pilkwang/birdclef-26-acoustic-time-window-rank-fusion?scriptVersionId=319935011) | expert | [Pilkwang Kim](https://www.kaggle.com/pilkwang) | South Korea |
# |||||||
# | [Model](#Model_5)_5 | [0.949](https://www.kaggle.com/code/sunderekkiz/birdclef-2026-exp019-eos4-rank-power-06?scriptVersionId=320064228) | v.1 | [birdclef 2026 exp019 eos4 rank power 06](https://www.kaggle.com/code/sunderekkiz/birdclef-2026-exp019-eos4-rank-power-06) | contributor | [Derek](https://www.kaggle.com/sunderekkiz) | World |
# |||||||
# 
# 
# | &nbsp;| &nbsp; | &nbsp; | &nbsp; | &nbsp; | &nbsp; | &nbsp; |
...
# |||||||
# || [0.949](https://www.kaggle.com/code/nina2025/birdclef-2026-eos-5?scriptVersionId=320192542) | &nbsp; v.5 | &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; [ 2 + 5 ] . [ 0.928 + 0.949 ] &nbsp; &nbsp; &nbsp; | [direct](#direct) | &nbsp; &nbsp; &nbsp;&nbsp; &nbsp;&nbsp; &nbsp; [[ 0.05 + 0.95 ](#preparing_parameters)] &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; | < 1.5 h
# || [0.949](https://www.kaggle.com/code/nina2025/birdclef-2026-eos-5?scriptVersionId=320202567) | &nbsp; v.6 | &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; [ 2 + 5 ] . [ 0.928 + 0.949 ] &nbsp; &nbsp; &nbsp; | [direct](#direct) | &nbsp; &nbsp; &nbsp;&nbsp; &nbsp;&nbsp; &nbsp; [[ 0.04 + 0.96 ](#preparing_parameters)] &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; | < 1.5 h
# || [?]() | &nbsp; v.7 | &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; [ 2 + 5 ] . [ 0.928 + 0.949 ] &nbsp; &nbsp; &nbsp; | [direct](#direct) | &nbsp; &nbsp; &nbsp;&nbsp; &nbsp;&nbsp; &nbsp; [[ 0.0321 + 0.9679 ](#preparing_parameters)] &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; | ?
# || [?]() | &nbsp; v.8 | &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; [ 2 + 5 ] . [ 0.928 + 0.949 ] &nbsp; &nbsp; &nbsp; | [direct](#direct) | &nbsp; &nbsp; &nbsp;&nbsp; &nbsp;&nbsp; &nbsp; &nbsp;&nbsp;&nbsp; [[ 0.0305 + 0.695 ](#preparing_parameters)] &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; |?
# 
# 
# v.5 < v.6
...
solutions = {
 'type_add' : 'direct',
 'Models'   : [
  {'Model':'Model_5a','subm':'subm_5a.csv','weight':0.500,'xSED':[0.60,0.40],'LB':'0.950'},
  {'Model':'Model_5b','subm':'subm_5b.csv','weight':0.500,'xSED':[0.59,0.41],'LB':'0.950'}
 ]
}
...
# ---- CELL ----
if 'Model_5a' in _ensemble_models:
            
    _file_name_submission = "subm_5a.csv"
    ...
    final_submission.to_csv('subm_5a.csv', index=False)
    
    display(final_submission.head(3))


# ---- CELL ----
if 'Model_5b' in _ensemble_models:
            
    _file_name_submission = "subm_5b.csv"
    ...
    if 'Karnakbayev_PowerOptimization_LB0948' in __ensemble_models:
        ...
        prior_tables   = build_prior_tables(sc, Y_SC)
        sc_te_adjusted = apply_prior(sc_te, sites=meta_te["site"].to_numpy(),
                                      hours=meta_te["hour_utc"].to_numpy(), tables=prior_tables, lambda_prior=0.60)
        ...
        sc_tr_prior = apply_prior(sc_tr, sites=meta_tr["site"].to_numpy(),
                                   hours=meta_tr["hour_utc"].to_numpy(), tables=prior_tables, lambda_prior=0.60)
        sc_tr_mlp = apply_mlp_probes_vectorized(emb_tr, sc_tr_prior, probe_models, emb_scaler, emb_pca, alpha_blend)
        first_pass_tr = (
            ENSEMBLE_W_PER_CLASS[None, :] * proto_tr_flat
            + (1.0 - ENSEMBLE_W_PER_CLASS)[None, :] * sc_tr_mlp
        ...
        probs = sigmoid(final_scores)
        probs = file_confidence_scale(probs, n_windows=N_WINDOWS, top_k=2, power=0.4)
        probs = rank_aware_scaling(probs,    n_windows=N_WINDOWS, power=0.60)
        probs = adaptive_delta_smooth(probs, n_windows=N_WINDOWS, base_alpha=0.20)
        probs = np.clip(probs, 0.0, 1.0)
        probs = apply_per_class_thresholds(probs, PER_CLASS_THRESHOLDS)   # ← now applied
    ...
    final_submission.to_csv('subm_5b.csv', index=False)
...
# def direct_blend(subm_1, wts, subm_2):
#     df1 = pd.read_csv(subm_1, index_col = "row_id")              ;display(df1)
#     df2 = pd.read_csv(subm_2, index_col = "row_id")[df1.columns] ;display(df2)
#     dfs = df1 * wts[0] + wts[1] * df2
#     return dfs
...
# ---- CELL ----
# === Final submission build + v221 TAX SMOOTHING POSTPROC safe version ===
import os
import pandas as pd
import numpy as np
...
# 0. Config
# ============================================================
APPLY_TAX_SMOOTHING = True

# v221 TAX SMOOTHING params
_ALPHA_GENUS = 0.15
_ALPHA_CLASS = 0.05
...
# ============================================================
# 1. Build submission from f_add()
# ============================================================
submission = f_add()

# ------------------------------------------------------------
# f_add() が None を返した場合の救済
# - 1モデル構成なら、そのモデルのCSVをそのまま読む
# - 2モデル以上で None なら direct()/f_add() 側の異常なので止める
# ------------------------------------------------------------
if submission is None:
    print("[WARN] f_add() returned None.")

    if "solutions" not in globals():
        raise RuntimeError("solutions is not defined, and f_add() returned None.")

    _models = solutions.get("Models", [])

    if len(_models) == 1:
        _single_file = _models[0].get("subm")

        if not _single_file:
            raise RuntimeError("single model fallback failed: 'subm' is missing in solutions.")

        if not os.path.exists(_single_file):
            raise FileNotFoundError(f"Fallback CSV not found: {_single_file}")

        print(f"[WARN] Falling back to single model CSV: {_single_file}")
        submission = pd.read_csv(_single_file)

    else:
        raise RuntimeError(
            f"f_add() returned None for {len(_models)} models. "
            "This means direct()/rank_1()/f_add() did not return a DataFrame. "
            "Fix the ensemble function or model CSV generation."
        )


# ============================================================
# 2. Normalize submission shape
#    - row_id を index にする
#    - species列だけを数値化
#    - Unnamed列を除去
# ============================================================
if isinstance(submission, pd.Series):
    raise TypeError("submission is Series, expected DataFrame.")

if not isinstance(submission, pd.DataFrame):
    raise TypeError(f"submission must be DataFrame, got {type(submission)}")

# row_id列があれば index に移す
...
# 事故で入った Unnamed列を除去
_bad_cols = [c for c in submission.columns if str(c).startswith("Unnamed")]
if _bad_cols:
    print(f"[WARN] Dropping accidental columns: {_bad_cols}")
    submission = submission.drop(columns=_bad_cols)

# 確率列を数値化
for _c in submission.columns:
    submission[_c] = pd.to_numeric(submission[_c], errors="coerce")

if submission.shape[1] == 0:
    raise ValueError("submission has no probability columns.")

_values = submission.values.astype(np.float32)

print(f"[pre] shape={submission.shape}")
print(f"[pre] mean={np.nanmean(_values):.6f}")
print(f"[pre] nan={np.isnan(_values).sum()}, finite={np.isfinite(_values).sum()}")

if not np.isfinite(_values).all():
    raise ValueError("submission contains NaN or inf before tax smoothing.")

if _values.min() < 0.0 or _values.max() > 1.0:
    print(f"[WARN] values outside [0,1] before smoothing: min={_values.min()}, max={_values.max()}")
    _values = np.clip(_values, 0.0, 1.0)
    submission.loc[:, :] = _values


# ============================================================
# 3. Optional TAX SMOOTHING
# ============================================================
if APPLY_TAX_SMOOTHING:
    print(f"[v221] TAX SMOOTHING enabled: genus alpha={_ALPHA_GENUS}, class alpha={_ALPHA_CLASS}")
    ...
    for _p in tax_paths:
        if os.path.exists(_p):
            _tax = pd.read_csv(_p)
            print(f"[v221] Loaded taxonomy: {_p}")
            break
    ...
    if _tax is None:
        ...
        for _c in _cols:
            _label = str(_c)
            _g = _species_to_genus.get(_label, _label)
            _cl = _species_to_class.get(_label, "")

            _genus_groups.setdefault(_g, []).append(_c)

            if _cl:
                _class_groups.setdefault(_cl, []).append(_c)

        # siblingが2個以上あるグループだけ smoothing
        _multi_genus = {
            _g: _members
            for _g, _members in _genus_groups.items()
            if len(_members) > 1
        }

        _multi_class = {
            _cl: _members
            for _cl, _members in _class_groups.items()
            if len(_members) > 1
        }

        print(f"[v221] multi-genus groups: {len(_multi_genus)}")
        print(f"[v221] multi-class groups: {len(_multi_class)}")
        ...
        for _g, _members in _multi_genus.items():
            ...
            probs[:, _idx] = (1.0 - _ALPHA_GENUS) * probs[:, _idx] + _ALPHA_GENUS * _genus_mean
        ...
        for _cl, _members in _multi_class.items():
            ...
            probs[:, _idx] = (1.0 - _ALPHA_CLASS) * probs[:, _idx] + _ALPHA_CLASS * _class_mean
        ...
        print(f"[v221] Post-tax-smoothing: mean={submission.values.mean():.6f}")
        print(f"[v221] Post-tax-smoothing: min={submission.values.min():.6f}, max={submission.values.max():.6f}")

else:
    print("[v221] TAX SMOOTHING disabled")
...
# 先頭列名を row_id に統一
if final_submission.columns[0] != "row_id":
    final_submission = final_submission.rename(columns={final_submission.columns[0]: "row_id"})

prob_cols = [c for c in final_submission.columns if c != "row_id"]
arr = final_submission[prob_cols].to_numpy(dtype=np.float32)

print("[final] shape:", final_submission.shape)
print("[final] prob_cols:", len(prob_cols))
print("[final] nan:", np.isnan(arr).sum())
print("[final] finite:", np.isfinite(arr).sum())
print("[final] min:", np.nanmin(arr) if np.isfinite(arr).any() else "NO_FINITE")
print("[final] max:", np.nanmax(arr) if np.isfinite(arr).any() else "NO_FINITE")
print("[final] Train rows:", final_submission["row_id"].astype(str).str.contains("_Train_").sum())
print("[final] Test rows :", final_submission["row_id"].astype(str).str.contains("_Test_").sum())

assert "row_id" in final_submission.columns, "row_id missing"
assert len(prob_cols) > 0, "no probability columns"
assert np.isfinite(arr).all(), "NaN or inf found in final submission"
assert arr.min() >= 0.0, "probability below 0"
assert arr.max() <= 1.0, "probability above 1"


# ============================================================
# 5. Optional sample_submission check
# ============================================================
sample_paths = [
...
for _p in sample_paths:
    if os.path.exists(_p):
        _sample = pd.read_csv(_p)
        break

if _sample is not None:
    same_shape = final_submission.shape == _sample.shape
    same_columns = final_submission.columns.tolist() == _sample.columns.tolist()
    same_row_id = final_submission["row_id"].astype(str).tolist() == _sample["row_id"].astype(str).tolist()

    print("[sample] shape:", _sample.shape)
    print("[sample] same shape:", same_shape)
    print("[sample] same columns:", same_columns)
    print("[sample] same row_id:", same_row_id)

    # columns が同じで row_id順だけ違う場合は sample順に並べ直す
    if same_columns and not same_row_id:
        _sample_ids = _sample["row_id"].astype(str).tolist()
        _current_ids = final_submission["row_id"].astype(str).tolist()

        if set(_sample_ids) == set(_current_ids):
            print("[sample] Reordering final_submission to sample_submission row_id order")
            final_submission["row_id"] = final_submission["row_id"].astype(str)
            final_submission = final_submission.set_index("row_id").loc[_sample_ids].reset_index()
            same_row_id = final_submission["row_id"].astype(str).tolist() == _sample_ids
            print("[sample] same row_id after reorder:", same_row_id)
        else:
            print("[WARN] row_id set differs from sample_submission. Not reordering.")
...
# === END Final submission build + v221 TAX SMOOTHING POSTPROC safe version ===
...
paths = sorted(Path("/kaggle/working").glob("*.csv"))

for path in paths:
    df = pd.read_csv(path)
    prob_cols = [c for c in df.columns if c != "row_id"]
    arr = df[prob_cols].to_numpy(dtype=np.float32) if prob_cols else np.array([])
    
    finite = int(np.isfinite(arr).sum()) if arr.size else 0
    nan = int(np.isnan(arr).sum()) if arr.size else 0
    
    print("\n", path.name)
    print("shape:", df.shape)
    print("prob_cols:", len(prob_cols))
    print("finite:", finite)
    print("nan:", nan)
    if finite > 0:
