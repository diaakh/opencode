import os, sys, math, time, json, random, gc, subprocess

# ---- P100 sm_60 fix: install torch cu126 BEFORE importing torch ----
def _fix_torch():
    try:
        import torch as _t
        # if already the right build with working sm_60, skip
        if _t.__version__.startswith("2.7.1"):
            return
    except Exception:
        pass
    if os.path.isdir("/kaggle"):
        print("Installing torch 2.7.1 cu126 (P100 sm_60 fix)...", flush=True)
...
# =====================================================================
CFG_BACKBONE = os.environ.get("SED_BACKBONE", "tf_efficientnet_b0")
CFG_RUN      = os.environ.get("SED_RUN", "b0")
CFG_SEED     = int(os.environ.get("SED_SEED", "42"))
